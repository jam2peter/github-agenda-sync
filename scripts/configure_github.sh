...
read -r -p "Repositories to sync (comma-separated owner/repo): " GH_AGENDA_REPOSITORIES
read -r -p "Google timezone [UTC]: " GOOGLE_TIMEZONE
...
echo
read -r -s -p "GitHub token with access to Issues: " GH_AGENDA_GITHUB_TOKEN
echo
